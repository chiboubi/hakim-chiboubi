...
import { 
  ...
  Sparkles, Zap, BrainCircuit, Waves, Radar
} from "lucide-react";
import { cn } from "@/lib/utils";
import { SCUGPHubUltimate } from "@/lib/scugp-service";
import { useToast } from "@/hooks/use-toast";

export const GeoIntelligenceDash = () => {
  const { toast } = useToast();
  const [mounted, setMounted] = useState(false);
  const [isProcessing, setIsProcessing] = useState(false);
  const metrics = SCUGPHubUltimate.getGeoAiMetrics();
  const boundaries = SCUGPHubUltimate.getFieldBoundaries();
  const buildings = SCUGPHubUltimate.getOpenBuildings();

  useEffect(() => {
    setMounted(true);
  }, []);

  const handleDeepSegmentation = () => {
    setIsProcessing(true);
    setTimeout(() => {
      setIsProcessing(false);
      toast({
        title: "Segmentation Geo AI Terminée",
        description: "Limites de concessions délimitées avec une précision de 0.5m."
      });
    }, 2500);
  };
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-1000">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: "Précision Boundary", val: metrics.boundary_precision, icon: Target, color: "text-emerald-400" },
          { label: "Confiance Seg.", val: metrics.segmentation_confidence, icon: BrainCircuit, color: "text-blue-400" },
          { label: "Sync OpenBuildings", val: metrics.openbuildings_sync, icon: Building2, color: "text-purple-400" },
          { label: "Standard OGC", val: "WMS/WFS", icon: ShieldCheck, color: "text-amber-400" }
        ].map((m, i) => (
        ...
        <div className="lg:col-span-8">
          <Card className="bg-black border-[12px] border-emerald-600 shadow-[0_0_200px_rgba(16,185,129,0.2)] rounded-[4rem] overflow-hidden relative text-white min-h-[700px]">
            ...
            <CardHeader className="p-16 border-b border-emerald-900/50 bg-emerald-600/10 text-center">
               ...
               <CardTitle className="text-7xl font-black uppercase tracking-[0.3em] italic leading-none">GEO-AI SOVEREIGNTY</CardTitle>
               <CardDescription className="text-emerald-400/60 font-bold uppercase tracking-[0.5em] mt-4">Field Boundary Detection & Open Geospatial Integration v117.0</CardDescription>
            </CardHeader>
            ...
            <CardContent className="p-16 space-y-16">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div className="p-10 bg-white/5 rounded-[3rem] border-2 border-emerald-500/20 space-y-6 shadow-inner text-center group hover:bg-emerald-500/10 transition-all">
                     ...
                     <h4 className="text-2xl font-black text-white uppercase tracking-widest">Field Boundaries</h4>
                  ...
                  <div className="p-10 bg-white/5 rounded-[3rem] border-2 border-blue-500/20 space-y-6 shadow-inner text-center group hover:bg-blue-500/10 transition-all">
                     ...
                     <h4 className="text-2xl font-black text-white uppercase tracking-widest">OpenBuildings Mesh</h4>
            ...
            <CardFooter className="p-12 bg-slate-950 border-t border-emerald-900/50 justify-between items-center">
               ...
               <Button onClick={handleDeepSegmentation} disabled={isProcessing} className="bg-emerald-600 hover:bg-emerald-700 text-black font-black h-16 px-16 rounded-[2rem] uppercase tracking-[0.5em] text-sm shadow-5xl border-none transition-all">
                 {isProcessing ? <Loader2 className="animate-spin mr-4" /> : <Radar className="mr-4" />}
          ...
          <Card className="bg-slate-900 border-slate-800 rounded-[3rem] shadow-3xl overflow-hidden h-full flex flex-col group">
            <CardHeader className="p-10 border-b border-white/5 bg-slate-950/50 text-center">
              <CardTitle className="text-sm font-black uppercase text-emerald-400 tracking-widest flex items-center justify-center gap-4">
                <Layers className="h-6 w-6 animate-pulse" /> Registre des Limites Ω
              </CardTitle>
            </CardHeader>
            <CardContent className="p-10 flex-1 space-y-6 overflow-y-auto max-h-[600px] scrollbar-thin scrollbar-thumb-emerald-900">
               <div className="space-y-6">
                  {boundaries.map((field) => (
                    <div key={field.id} className="p-6 bg-black/40 rounded-[2.5rem] border border-white/5 group hover:border-blue-500/30 transition-all shadow-xl">
                       ...
                       <p className="text-[9px] text-slate-500 font-bold uppercase mt-2">Précision: 0.5m</p>
                    </div>
                  ))}
               </div>
