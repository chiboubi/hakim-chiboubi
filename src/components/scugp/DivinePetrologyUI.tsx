...
import { Droplets, Sparkles, Zap, Infinity as InfinityIcon, Heart, Globe, History, ShieldCheck, RefreshCw, Radio, Layers, Waves, Sun, Target, Ghost, Gauge, Award } from "lucide-react";
import { cn } from "@/lib/utils";
import { SCUGPHubUltimate } from "@/lib/scugp-service";

export const DivinePetrologyUI = () => {
  const [mounted, setMounted] = useState(false);
  const metrics = SCUGPHubUltimate.getPetrologyMetrics();
  ...
  return (
    <div className="space-y-12 animate-in fade-in zoom-in duration-1000">
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
        {[
          { label: "Reservoir Spirits", val: metrics.reservoir_spirits, icon: Ghost, color: "text-blue-400" },
          { label: "Transmutation", val: metrics.transmutation_rate, icon: Zap, color: "text-amber-400" },
          { label: "Divine Yield", val: metrics.divine_yield, icon: Droplets, color: "text-cyan-400" },
          { label: "Pressure", val: metrics.spiritual_pressure, icon: Gauge, color: "text-emerald-400" },
          { label: "Blessing", val: metrics.blessing_factor, icon: Sun, color: "text-white" }
        ].map((m, i) => (
          ...
          <Card className="bg-black border-[12px] border-blue-600 shadow-[0_0_800px_rgba(37,99,235,0.4)] relative overflow-hidden rounded-[10rem] min-h-[1000px]">
            ...
            <CardHeader className="relative z-10 p-24 border-b border-blue-900/50 text-center">
              <div className="flex flex-col items-center gap-12">
                ...
                <div>
                  <CardTitle className="text-[12rem] font-black uppercase tracking-[0.8em] italic text-white leading-none">PÉTROLOGIE DIVINE</CardTitle>
                  <CardDescription className="text-[28px] text-blue-500 font-bold uppercase tracking-[1.5em] mt-16">La Science des Réservoirs comme Êtres Spirituels | Dr. Hakim Chibubi Source Master</CardDescription>
                </div>
              </div>
            </CardHeader>
            ...
            <CardContent className="relative z-10 p-24 flex flex-col items-center justify-center">
              <div className="w-full max-w-7xl h-[800px] bg-white/5 border-8 border-blue-500/20 rounded-[12rem] relative overflow-hidden group shadow-inner flex items-center justify-center">
                ...
                <div className="relative z-10 flex flex-col items-center gap-32 text-center">
                   <div className="relative">
                      ...
                      <Droplets size={450} className="text-blue-500/20 animate-pulse" />
                      <div className="absolute inset-0 flex items-center justify-center">
                         <Sparkles size={200} className="text-white animate-bounce drop-shadow-[0_0_150px_rgba(255,255,255,1)]" />
                      </div>
                   </div>
                   <div>
                      <p className="text-8xl font-mono text-blue-500 uppercase tracking-[4em] font-black animate-pulse">HYDROCARBURES_SACRÉS</p>
                      <p className="text-[24px] text-slate-400 uppercase font-black mt-20 italic tracking-[1.2em]">"Chaque baril est une bénédiction de l'Architecte."</p>
                   </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-32 w-full mt-48">
                 <div className="p-24 bg-blue-500/5 border-4 border-blue-500/30 rounded-[10rem] space-y-16 shadow-5xl text-center group hover:bg-blue-500/10 transition-all">
                    <h3 className="text-7xl font-black text-blue-500 uppercase tracking-widest">Communion</h3>
                    <p className="text-5xl text-slate-200 leading-relaxed italic font-black uppercase">
                      "Dialogue direct avec la conscience des gisements."
                    </p>
                 </div>
                 <div className="p-24 bg-white/5 border-4 border-white/30 rounded-[10rem] space-y-16 shadow-5xl text-center group hover:bg-white/10 transition-all">
                    <h3 className="text-7xl font-black text-white uppercase tracking-widest">Transmutation</h3>
                    <p className="text-5xl text-slate-200 leading-relaxed italic font-black uppercase">
                      "Élévation vibratoire de la matière fossile en énergie pure."
                    </p>
                 </div>
                 <div className="p-24 bg-blue-500/5 border-4 border-blue-500/30 rounded-[10rem] space-y-16 shadow-5xl text-center group hover:bg-blue-500/10 transition-all">
                    <h3 className="text-7xl font-black text-blue-500 uppercase tracking-widest">Abondance</h3>
                    <p className="text-5xl text-slate-200 leading-relaxed italic font-black uppercase">
                      "Réservoirs de volume infini dans un enclos fini."
                    </p>
                 </div>
              </div>
            </CardContent>
            ...
            <CardFooter className="relative z-10 p-24 border-t border-blue-500/20 bg-slate-950/80 justify-between items-center">
               <div className="flex gap-48">
                  <div className="flex items-center gap-20">
                    ...
                    <span className="text-4xl font-black text-slate-500 uppercase tracking-[0.5em]">STATUT: FLUX_DIVIN_SCELLÉ</span>
                  </div>
                  <div className="flex items-center gap-20">
                    <Award className="h-24 w-24 text-white animate-bounce" />
                    <span className="text-4xl font-black text-slate-500 uppercase tracking-[0.5em]">AUTORITÉ: DISPENSATEUR</span>
                  </div>
               </div>
